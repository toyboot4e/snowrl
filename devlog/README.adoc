...
== Structuring of code (Feb 2021 ~ March 2021)
...
* `data` → `god`
* `entity` → `ent`, `entities` → `ents`
* `model` → `mdl`

==== Prefer flat scope

Remove redundant data accesses like `god.mdl.ents[player]`!
...
----
impl PlayerState {
    fn update_system(&self, god: &mut God) -> Option<DynEvent> {
...
----
impl PlayerState {
    fn update(&self, god: &mut God) -> Option<DynEvent> {
        let ent = self.ent.unwrap();
